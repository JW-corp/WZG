...
basedir = '/x5/cms/jwkim/gitdir/JWCorp/JW_analysis/Coffea_WZG/Condor_coffea/Fitting_210810_FakePhoton_ClosureTest_lowptbins'
...
for PT_eta_bin in PT_eta_bin:
	...
	df_path = basedir + '/' + PT_eta_bin+ '/log.csv'
	df = pd.read_csv(df_path,delimiter='\s+',header=None,names=['bins_name','SB','Fake_fraction','unc'])
	...
	df_pivoted = df.pivot('high IsoChg','low IsoChg','unc_closure')
	print(df_pivoted)
	
	# draw heat,ap
	plt.rcParams["figure.figsize"] = (8,8)
	plt.rcParams.update({'font.size': 15})
	ax = sns.heatmap(df_pivoted,annot=True,linewidths=1,cmap='Blues',fmt='.3f',linecolor='lightsteelblue')
	ax.invert_yaxis()
