...
class JW_Processor(processor.ProcessorABC):
		...
		self._accumulator = processor.dict_accumulator(
			{
				...
				"PT_1_eta_2": hist.Hist(
					...
					hist.Cat("dataset", "Dataset"),
					hist.Bin(
						"PT_1_eta_2", "20 < pt <30 & Endcap", 200, 0, 0.075
					),
				),
				...
				"PT_2_eta_2": hist.Hist(
					...
					hist.Cat("dataset", "Dataset"),
					hist.Bin(
						"PT_2_eta_2", "30 < pt <40 & Endcap", 200, 0, 0.075
					),
				),
				"PT_3_eta_1": hist.Hist(
					"Events",
					hist.Cat("dataset", "Dataset"),
					hist.Bin("PT_3_eta_1", "40 < pt & Barrel", 200, 0, 0.02),
				),
				"PT_3_eta_2": hist.Hist(
					"Events",
					hist.Cat("dataset", "Dataset"),
					hist.Bin(
						"PT_3_eta_2", "40 < pt & Endcap", 200, 0, 0.075
					),
				),
			}
		)
	...
	def process(self, events):
		...
		Diele_base = Diele[Event_sel_mask]
		leading_pho_base = leading_pho[Event_sel_mask]
		Jet_base = Jet[Event_sel_mask]
		MET_base = MET[Event_sel_mask]
		...
		for name in bin_name_list:
			binned_sieie_hist[name] = make_bins(
				...
				ak.flatten(leading_pho_base.isScEtaEB),
				ak.flatten(leading_pho_base.isScEtaEE),
...
if __name__ == "__main__":
	...
	outname = 'Data_template_' + data_sample + ".futures"
