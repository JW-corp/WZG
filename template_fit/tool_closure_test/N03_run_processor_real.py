...
class JW_Processor(processor.ProcessorABC):
		...
		self._accumulator = processor.dict_accumulator(
			{
				...
				"PT_1_eta_1": hist.Hist(
					...
					hist.Bin("PT_1_eta_1", "20 < pt <30 & Barrel", 200, 0, 0.02),
				),
				"PT_1_eta_2": hist.Hist(
					"Events",
					hist.Cat("dataset", "Dataset"),
					hist.Bin("PT_1_eta_2", "20 < pt <30 & Endcap", 200, 0, 0.075),
				),
				"PT_2_eta_1": hist.Hist(
					"Events",
					hist.Cat("dataset", "Dataset"),
					hist.Bin("PT_2_eta_1", "30 < pt <40 & Barrel", 200, 0, 0.02),
				),
				"PT_2_eta_2": hist.Hist(
					"Events",
					hist.Cat("dataset", "Dataset"),
					hist.Bin("PT_2_eta_2", "30 < pt <40 & Endcap", 200, 0, 0.075),
				),
				"PT_3_eta_1": hist.Hist(
					"Events",
					hist.Cat("dataset", "Dataset"),
					hist.Bin("PT_3_eta_1", "40 < pt <50 & Barrel", 200, 0, 0.02),
				),
				"PT_3_eta_2": hist.Hist(
					"Events",
					hist.Cat("dataset", "Dataset"),
					hist.Bin("PT_3_eta_2", "40 < pt <50 & Endcap", 200, 0, 0.075),
				),
				#"PT_4_eta_1": hist.Hist(
				#	"Events",
				#	hist.Cat("dataset", "Dataset"),
				#	hist.Bin("PT_4_eta_1", "50 < pt & Barrel", 200, 0, 0.02),
				#),
				#"PT_4_eta_2": hist.Hist(
				#	"Events",
				#	hist.Cat("dataset", "Dataset"),
				#	hist.Bin("PT_4_eta_2", "50 <pt  & Endcap", 200, 0, 0.075),
				#),
			}
		)
	...
	def process(self, events):
		...
		PhoSelmask = (
			PT_mask & isgap_mask & Pixel_seed_mask & dr_pho_ele_mask & dr_pho_mu_mask
		)
		...
		if not isData:
			pu = pu[gen_match_photon_evt_mask]
		...
		# -------------------- Sieie bins---------------------------#
		def make_bins(pt, isEB, isEE, sieie, bin_range_str):
		...
		for name in bin_name_list:
			binned_sieie_hist[name], _ = make_bins(
				...
				ak.flatten(leading_pho_sel.isScEtaEB),
				ak.flatten(leading_pho_sel.isScEtaEE),
				ak.flatten(leading_pho_sel.sieie),
				name,
			)

			_, binmask_dict[name] = make_bins(
		...
		print("Show me the last bin: ", binned_sieie_hist["PT_3_eta_2"])
		...
		print(
			"cut0: {0}, cut1: {1}, cut2: {2}, cut3: {3}, cut4: {4} ,cut5 {5} ".format(
				len(Initial_events),
				len(cut1),
				len(cut2),
				len(cut3),
...
if __name__ == "__main__":
	...
	parser.add_argument("--year", type=str, help="--year 2018", default="2018")
	...
	if not isdata:
		...
		if year == "2018":
			pu_path = (
				"../Corrections/Pileup/puWeight/npy_UL_Run2018/"
