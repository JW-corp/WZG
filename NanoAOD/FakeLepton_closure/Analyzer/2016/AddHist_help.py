...
def AddHist_FakeLepton(file, hist, isData, xsec, lumi, channel, branch,realFake=False):
	...
	if isData:
		...
		#		#hist[branch_name].Fill(float(arrays[branch[branch_name]["name"]].values[i]), -1 * float(arrays['fake_lepton_closure_weight'].values[i]) * float(arrays['true_weight'].values[i]))
