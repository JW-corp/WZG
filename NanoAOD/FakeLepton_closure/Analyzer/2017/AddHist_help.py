...
def channel_cut(channel, arrays):
	...
	if channel in [0,1,2,3,4]:
		print("BEFORE SR:",len(arrays))
		sel = '(mwa<75|mwa>105) & WZG_trileptonmass>100 & (WZG_dileptonmass>75&WZG_dileptonmass<105)'
		#sel = ' WZG_trileptonmass>100 & (WZG_dileptonmass>75&WZG_dileptonmass<105)'
		arrays = arrays.query(sel)
		print("AFTER SR:",len(arrays))
...
def lep_gen_cut(channel, arrays,realFake=False):

	# Real fake photon based-on : one of leptons is fake
	if realFake:
		true_WZG_lepton1 = ((arrays.loc[:,'WZG_lepton1_genPartFlav'] == 1) | (arrays.loc[:,'WZG_lepton1_genPartFlav'] == 15))
		true_WZG_lepton2 = ((arrays.loc[:,'WZG_lepton2_genPartFlav'] == 1) | (arrays.loc[:,'WZG_lepton2_genPartFlav'] == 15))
		true_WZG_lepton3 = ((arrays.loc[:,'WZG_lepton3_genPartFlav'] == 1) | (arrays.loc[:,'WZG_lepton3_genPartFlav'] == 15))

		true_ttG_lepton1 = ((arrays.loc[:,'ttG_lepton1_genPartFlav'] == 1) | (arrays.loc[:,'ttG_lepton1_genPartFlav'] == 15))
		true_ttG_lepton2 = ((arrays.loc[:,'ttG_lepton2_genPartFlav'] == 1) | (arrays.loc[:,'ttG_lepton2_genPartFlav'] == 15))
		true_ttG_lepton3 = ((arrays.loc[:,'ttG_lepton3_genPartFlav'] == 1) | (arrays.loc[:,'ttG_lepton3_genPartFlav'] == 15))


		true_ZGJ_lepton1 = ((arrays.loc[:,'ZGJ_lepton1_genPartFlav'] == 1) | (arrays.loc[:,'ZGJ_lepton1_genPartFlav'] == 15))
		true_ZGJ_lepton2 = ((arrays.loc[:,'ZGJ_lepton2_genPartFlav'] == 1) | (arrays.loc[:,'ZGJ_lepton2_genPartFlav'] == 15))

		

		lep_gen_cut_WZG = ((true_WZG_lepton1 & true_WZG_lepton2 & true_WZG_lepton3) ==False)
		lep_gen_cut_ttG = ((true_ttG_lepton1 & true_ttG_lepton2 & true_ttG_lepton3) ==False)
		lep_gen_cut_ZGJ = ((true_ZGJ_lepton1 & true_ZGJ_lepton2 ) ==False)




	# Estimated fake : Use region-mark ==2 (FakeLep AR) and Fake lepton AR
	else:
	...
	if channel in [10,11,12,13,14,5,6,7,8,9]:
		...
		if realFake:
			true_ttZ_lepton1 =  ((arrays.loc[:,'ttZ_lepton1_genPartFlav'] == 1) | (arrays.loc[:,'ttZ_lepton1_genPartFlav'] == 15))
			true_ttZ_lepton2 =  ((arrays.loc[:,'ttZ_lepton2_genPartFlav'] == 1) | (arrays.loc[:,'ttZ_lepton2_genPartFlav'] == 15))
			true_ttZ_lepton3 =  ((arrays.loc[:,'ttZ_lepton3_genPartFlav'] == 1) | (arrays.loc[:,'ttZ_lepton3_genPartFlav'] == 15))
		

			true_ZZ_lepton1 =  ((arrays.loc[:,'ZZ_lepton1_genPartFlav'] == 1) | (arrays.loc[:,'ZZ_lepton1_genPartFlav'] == 15))
			true_ZZ_lepton2 =  ((arrays.loc[:,'ZZ_lepton2_genPartFlav'] == 1) | (arrays.loc[:,'ZZ_lepton2_genPartFlav'] == 15))
			true_ZZ_lepton3 =  ((arrays.loc[:,'ZZ_lepton3_genPartFlav'] == 1) | (arrays.loc[:,'ZZ_lepton3_genPartFlav'] == 15))
			true_ZZ_lepton4 =  ((arrays.loc[:,'ZZ_lepton4_genPartFlav'] == 1) | (arrays.loc[:,'ZZ_lepton4_genPartFlav'] == 15))


			lep_gen_cut_ttZ = ((true_ttZ_lepton1 & true_ttZ_lepton2 & true_ttZ_lepton3) ==False)
			lep_gen_cut_ZZ  = ((true_ZZ_lepton1 & true_ZZ_lepton2 & true_ZZ_lepton3 & true_ZZ_lepton4) ==False)
...
def AddHist(file, hist, isData, xsec, lumi, channel, branch):
	...
	if isData:
		...
		if file_basename == "LLWA_aQGC_UL17_Skim.root":
	...
	aQGC_photon_pt_cut = arrays.loc[:,'WZG_photon_pt'] > 60
	arrays = arrays.loc[aQGC_photon_pt_cut,:]
	# <-----  -JiWoong-
	...
	if isData:
		if channel in [0,1,2,3,4, 10,11,12,13,14, 20,21,22,23,24]:
		...
		if file_basename == "LLWA_aQGC_UL17_Skim.root":
			...
			for i in range(len_weight):
				...
				xsec = arrays["LHEReweightingWeight"][:,i] * arrays["LHEWeight_originalXWGTUP"]
		

				# skip b-tag weight -new mehtod will be implemented-
				#arrays[f'true_weight_Rwt{i}'] = arrays['puWeight'] * arrays['L1PreFiringWeight_Nom'] * btag_weight_ratio * arrays['btagWeight'] * arrays['Muon_ID_Weight'] * arrays['Electron_ID_Weight'] * arrays['Electron_RECO_Weight'] * lumi * xsec * 1000 * arrays['Generator_weight_sgn'] / true_events
				arrays[f'true_weight_Rwt{i}'] = arrays['puWeight'] * arrays['L1PreFiringWeight_Nom'] * arrays['Muon_ID_Weight'] * arrays['Electron_ID_Weight'] * arrays['Electron_RECO_Weight'] * lumi * xsec * 1000 * arrays['Generator_weight_sgn'] / true_events
				...
				for branch_name in branch:
					...
					if branch_name != "WZG_mllla":
						continue
					# <<<arrays_copy_nominal
					print(arrays_copy_nominal[branch[branch_name]["name"]][:,0])

					# Index [:,0] becuase of the broad-casted same 193 number of sub-array
					for j in trange(0, len(arrays_copy_nominal[branch[branch_name]["name"]][:,0]), desc=f'fill {branch[branch_name]["name"]} for {file} in {UpDown_map[0]}_Rwt{i}'):
						hist[branch_name+f"_{UpDown_map[0]}_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal[f'true_weight_Rwt{i}'][:,0].values[j]))
				print("\n")
				...
				arrays_copy_nominal['true_weight_l1up_Rwt{i}'] = arrays_copy_nominal['puWeight'] * arrays_copy_nominal['L1PreFiringWeight_Up'] * arrays_copy_nominal['Muon_ID_Weight'] * arrays_copy_nominal['Electron_ID_Weight'] * arrays_copy_nominal['Electron_RECO_Weight'] * lumi * xsec * 1000 * arrays_copy_nominal['Generator_weight_sgn'] / true_events
				arrays_copy_nominal['true_weight_l1down_Rwt{i}'] = arrays_copy_nominal['puWeight'] * arrays_copy_nominal['L1PreFiringWeight_Dn'] * arrays_copy_nominal['Muon_ID_Weight'] * arrays_copy_nominal['Electron_ID_Weight'] * arrays_copy_nominal['Electron_RECO_Weight'] * lumi * xsec * 1000 * arrays_copy_nominal['Generator_weight_sgn'] / true_events
				arrays_copy_nominal['true_weight_puup_Rwt{i}'] = arrays_copy_nominal['puWeightUp'] * arrays_copy_nominal['L1PreFiringWeight_Nom'] * arrays_copy_nominal['Muon_ID_Weight'] * arrays_copy_nominal['Electron_ID_Weight'] * arrays_copy_nominal['Electron_RECO_Weight'] * lumi * xsec * 1000 * arrays_copy_nominal['Generator_weight_sgn'] / true_events
				arrays_copy_nominal['true_weight_pudown_Rwt{i}'] = arrays_copy_nominal['puWeightDown'] * arrays_copy_nominal['L1PreFiringWeight_Nom'] * arrays_copy_nominal['Muon_ID_Weight'] * arrays_copy_nominal['Electron_ID_Weight'] * arrays_copy_nominal['Electron_RECO_Weight'] * lumi * xsec * 1000 * arrays_copy_nominal['Generator_weight_sgn'] / true_events
				for branch_name in branch:
					...
					for j in trange(0, len(arrays_copy_nominal[branch[branch_name]["name"]][:,0]), desc=f'fill {branch[branch_name]["name"]} for {file} in {UpDown_map[0]}_Rwt{i}'):
						hist[branch_name+f"_MuonIDup_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal['true_weight_MuonIDup_Rwt{i}'][:,0].values[j]))
						hist[branch_name+f"_MuonIDdown_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal['true_weight_MuonIDdown_Rwt{i}'][:,0].values[j]))
						hist[branch_name+f"_ElectronIDup_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal['true_weight_ElectronIDup_Rwt{i}'][:,0].values[j]))
						hist[branch_name+f"_ElectronIDdown_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal['true_weight_ElectronIDdown_Rwt{i}'][:,0].values[j]))
						hist[branch_name+f"_ElectronRECOup_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal['true_weight_ElectronRECOup_Rwt{i}'][:,0].values[j]))
						hist[branch_name+f"_ElectronRECOdown_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal['true_weight_ElectronRECOdown_Rwt{i}'][:,0].values[j]))
						hist[branch_name+f"_l1up_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal['true_weight_l1up_Rwt{i}'][:,0].values[j]))
						hist[branch_name+f"_l1down_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal['true_weight_l1down_Rwt{i}'][:,0].values[j]))
						hist[branch_name+f"_puup_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal['true_weight_puup_Rwt{i}'][:,0].values[j]))
						hist[branch_name+f"_pudown_Rwt{i}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy_nominal['true_weight_pudown_Rwt{i}'][:,0].values[j]))
				print("\n")
				...
				for UpDown in range(1,5):
					...
					for branch_name in branch:
						...
						x = len(arrays_copy[branch[branch_name]["name"]][:,0])
						print(f"There are {x} number of events")
						for j in trange(0, len(arrays_copy[branch[branch_name]["name"]][:,0]), desc=f'fill {branch[branch_name]["name"]} for {file} in {UpDown_map[UpDown]}_Rwt{i}'):
							hist[branch_name+f"_{UpDown_map[UpDown]}_Rwt{i}"].Fill(float(arrays_copy[branch[branch_name]["name"]][:,0].values[j]), float(arrays_copy[f'true_weight_Rwt{i}'][:,0].values[j]))
		...
		#------------->  Non-Reweight
			...
			for UpDown in range(1,5):
				...
				if channel in [0,1,2,3,4, 10,11,12,13,14, 20,21,22,23,24, 30,31,32]:
					MET_cut = (arrays.loc[:,f'MET_T1Smear_pt_{UpDown_map[UpDown]}'] > 30)
					arrays_copy = arrays.loc[MET_cut,:].copy()
					print(",,",len(arrays),len(arrays_copy))
				elif channel in [5,6,7,8,9]:
					MET_cut = (arrays.loc[:,f'MET_T1Smear_pt_{UpDown_map[UpDown]}'] <= 30)
					arrays_copy = arrays.loc[MET_cut,:].copy()
				else:
				...
				for branch_name in branch:
					...
					if branch_name != "WZG_mllla":
						continue
					# <<<			
					x = print(len(arrays_copy[branch[branch_name]["name"]]))
					print(arrays_copy[branch[branch_name]["name"]])
					print(f"There are funcking {x} number of events")	
					for i in trange(0, len(arrays_copy[branch[branch_name]["name"]]), desc=f'fill {branch[branch_name]["name"]} for {file} in {UpDown_map[UpDown]}'):
						...
						print("SHOWME",arrays_copy[branch[branch_name]["name"]].values[i],arrays_copy['true_weight'].values[i])
...
def AddHist_FakeLepton(file, hist, isData, xsec, lumi, channel, branch,realFake=False):
	
	init_time = time.time()
	#init_branches = ['fake_lepton_weight',\
	init_branches = ['fake_lepton_closure_weight',\
					'channel_mark',\
					'region_mark',\
					'HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL',\
					'HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_DZ',\
	...
	if isData:
		...
		for branch_name in branch:
			for i in trange(0, len(arrays[branch[branch_name]["name"]]), desc=f'fill {branch[branch_name]["name"]} for {file}'):
				...
				hist[branch_name].Fill(float(arrays[branch[branch_name]["name"]].values[i]), float(arrays['fake_lepton_closure_weight'].values[i]))
			print (f"SumOfWeights for {branch_name}: ", hist[branch_name].GetSumOfWeights())
	else:


		# Only apply gen-particle cut to for True-fake
		## >> Added
		#if realFake:
		#	arrays = pho_gen_cut(channel, arrays)
		#	arrays = lep_gen_cut(channel, arrays, realFake)
		## << 

		# Just for test: apply both lepton-gen cut and photon-gen cut 
		arrays = pho_gen_cut(channel, arrays)
		#arrays = lep_gen_cut(channel, arrays, realFake)

		# both are basically at least one fake (loos-but-not-tight)
		arrays = lep_gen_cut(channel, arrays, True)
		...
		if realFake:
			...
			for branch_name in branch:
				for i in trange(0, len(arrays_copy_nominal[branch[branch_name]["name"]]), desc=f'fill {branch[branch_name]["name"]} for {file} in {UpDown_map[0]}'):
					#hist[branch_name+f"_{UpDown_map[0]}"].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]].values[i]), float(arrays_copy_nominal['true_weight'].values[i]))
					hist[branch_name].Fill(float(arrays_copy_nominal[branch[branch_name]["name"]].values[i]), float(arrays_copy_nominal['true_weight'].values[i]))
			print (f"SumOfWeights for {branch_name}: ", hist[branch_name].GetSumOfWeights())
			print("\n")
...
#
#
#				hist[branch_name].Fill(float(arrays[branch[branch_name]["name"]].values[i]),float(arrays['fake_lepton_closure_weight'].values[i]) * float(arrays['true_weight'].values[i]))
#
#			print (f"SumOfWeights for {branch_name}: ", hist[branch_name].GetSumOfWeights())
#	
#	end_time = time.time()
