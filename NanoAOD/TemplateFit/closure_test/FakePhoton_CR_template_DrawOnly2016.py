...
if __name__ == "__main__":
	...
	parser.add_argument('sb', type=str,
				help="from_4_to_10 .." )
	parser.add_argument('--closure', type=bool,
	...
	if closure:
		hist_datafake  = hist_datafake_pickle[sb]['histo'][region]
		true_fake_frac = hist_datafake_pickle[sb]['true_fake_fraction'][region]
	else:
	...
	if closure:
		outname = sb + '_' +  region +'_AfterFit.png'
	else:
		outname = region + '_AfterFit.png'


	outdir='fit_out_images_AfterFit'
	Path(outdir).mkdir(exist_ok=True,parents=True)
	outpath = outdir + '/' + outname
	c1.Print(outpath)
	...
	print(f"Fake Fraction: {fake_fraction} +- {fake_fraction_err}")

	if closure:
		SB_unc = (true_fake_frac - fake_fraction) / true_fake_frac
		
		sb_lower = sb.split('_')[1]
		sb_upper = sb.split('_')[3]
		f = open('log.csv','a')
		f.write("{0} {1} {2} \n".format(sb_lower,sb_upper,SB_unc))
		f.close()
