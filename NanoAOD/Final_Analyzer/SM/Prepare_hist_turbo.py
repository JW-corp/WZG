...
class WZG_plot():
		...
		if isData:
			...
			for branch_name in self.branch:
				...
				for unc in self.unc_map:
					...
					## JUST FOR CHECK
					#print("Drawing btag plot...")
					#if "btag" in unc:
					#	plt.hist(df[f'{unc}Up'],bins=50,label=f'{branch_name}_{unc}')
					#	plt.savefig(f'{branch_name}_{unc}.png')
					#	plt.close()
					## JUST FOR CHECK	
	...
	def plot(self, hists_data={}, hists_mcgroup={}, hists_flep={}, hists_fpho={}, flow='none', **kwargs):
		...
		for branch_name in self.branch:
			...
			# >> Add to show yields
			if branch_name == "WZG_mlla":
				print("FakePhoton: ",hists_fpho[branch_name].sum().value,hists_fpho[branch_name].sum().variance)
				print("FakeLepton: ",hists_flep[branch_name].sum().value,hists_flep[branch_name].sum().variance)
				for group in self.plot_groups:
					print(group,": ",hists_mcgroup[group][branch_name].sum().value,hists_mcgroup[group][branch_name].sum().variance)
				print("Data: ",hists_data[branch_name].sum().value,hists_data[branch_name].sum().variance)
			# <<
			...
			hep.cms.label(data=True, lumi=self.lumi, year=self.year, ax=axs[0])
...
queue 1'''
	...
	def run(self, mode='prepare', iscondor=False, input=None, **kwargs):
		...
		if mode.lower() == 'prepare':
			...
			file = uproot.open(f'root_store/{self.region}_{self.year}.root')
			for group in self.plot_groups:
