...
if __name__ == "__main__":
	...
	# SB  unc --		
	#with open('pickle_dict_sample/fake_hist_high_dict_18.pickle','rb') as fr:
	#	hist_datafake_pickle   = pickle.load(fr)

	#with open('pickle_dict_sample/fake_hist_low_dict_18.pickle','rb') as fr:
	#	hist_datafake_pickle   = pickle.load(fr)
	...
	if closure:
		outname = sb + '_' +  region +'_AfterFit_18.png'
	else:
		outname = region + '_AfterFit_18.png'

	
	outdir='fit_out_images_AfterFit_18'
	Path(outdir).mkdir(exist_ok=True,parents=True)
	outpath = outdir + '/' + outname
	c1.Print(outpath)
