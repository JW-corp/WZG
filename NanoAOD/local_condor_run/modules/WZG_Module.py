...
class WZG_Producer(Module):
	...
	def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
		...
		self.out.branch("WZG_mla",  "F")
		self.out.branch("WZG_mWZ",  "F")
		self.out.branch("WZG_maWZ",  "F")
	...
	def analyze(self, event):
		...
		for i in range(0,len(muons)):
			if event.Muon_corrected_pt[i] < min_pt_lep_cut:
				continue
		...
		if len(tight_electrons) + len(tight_muons) == 3:
			...
			m_lla = -float('inf')
			m_la  = -float('inf') # third lepton + gamma mass to exclude 4th lepton->photon conversion
			m_WZ  = -float('inf') # m(WZ) for aQGC limit
			m_aWZ = -float('inf') # m(WZG) for aQGC limit
			...
			if len(tight_muons)==2 and len(tight_electrons)==1:  # emumu channel 
				if muons[tight_muons[0]].pdgId == -muons[tight_muons[1]].pdgId:
					...
					if len(tight_photons) > 0:
						...
						m_la   = (electrons[tight_electrons[0]].p4() + photons[tight_photons[0]].p4()).M()
			...
			if len(tight_muons)==1 and len(tight_electrons)==2:
				if electrons[tight_electrons[0]].pdgId == -electrons[tight_electrons[1]].pdgId:
					...
					if len(tight_photons) > 0:
						...
						m_la   = (muons[tight_muons[0]].p4() + photons[tight_photons[0]].p4()).M()
			...
			if len(tight_electrons)==3 and len(tight_muons)==0:
				...
				if len(tight_photons) > 0:
					...
					m_la   = (electrons[tight_electrons[0]].p4() + photons[tight_photons[0]].p4()).M() # Wlepton and photon mass 
			...
			if len(tight_muons)==3 and len(tight_electrons)==0:
				...
				if len(tight_photons) > 0:
					...
					m_la   = (muons[tight_muons[0]].p4() + photons[tight_photons[0]].p4()).M() # Wlepton and photon mass 
			...
			if len(tight_bjets) == 0:
				...
				#self.out.fillbranch("WZG_mla",  m_la)
...
WZG_select_Module_16preVFP  = lambda : WZG_Producer("2016_preVFP")
