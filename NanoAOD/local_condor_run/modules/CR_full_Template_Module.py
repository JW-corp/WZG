...
class CR_FakePhotonFullProducer(Module):
	...
	def analyze(self, event):
		...
		for i in range(0, len(electrons)):
			if electrons[i].pt < min_pt_lep_cut:
				continue
		...
		for i in range(0, len(photons)):
			...
			if not (
				(abs(photons[i].eta) < 1.4442)
				or (1.566 < abs(photons[i].eta) and abs(photons[i].eta) < 2.5)
			):
				continue
			...
			for j in range(0, len(tight_muons)):
				if (
					deltaR(
						muons[tight_muons[j]].eta,
						muons[tight_muons[j]].phi,
						photons[i].eta,
						photons[i].phi,
					)
					< 0.5
			...
			for j in range(0, len(tight_electrons)):
				if (
					deltaR(
						electrons[tight_electrons[j]].eta,
						electrons[tight_electrons[j]].phi,
						photons[i].eta,
						photons[i].phi,
					)
					< 0.5
			...
			mask_iso_sigmaietaieta = (
				(1 << 1) | (1 << 3) | (1 << 5) | (1 << 11) | (1 << 13)
			)  # remove charge iso and sigmaietaieta cut in medium id
			...
			for j in range(0, len(tight_photons)):
				if (
					deltaR(
						jets[i].eta,
						jets[i].phi,
						photons[tight_photons[j]].eta,
						photons[tight_photons[j]].phi,
					)
					< 0.5
				):
					pass_lepton_dr_cut = False
		...
		if len(tight_electrons) + len(tight_muons) == 2:
			if len(tight_electrons) == 2:
				dileptonmass = (
					electrons[tight_electrons[0]].p4()
					+ electrons[tight_electrons[1]].p4()
				).M()
			elif len(tight_muons) == 2:
				dileptonmass = (
					muons[tight_muons[0]].p4() + muons[tight_muons[1]].p4()
				).M()
			else:
				dileptonmass = (
					muons[tight_muons[0]].p4() + electrons[tight_electrons[0]].p4()
				).M()
			...
			self.out.fillBranch(
				"photon_pt", photons[tight_without_iso_sigmaietaieta_photons[0]].pt
			)
			self.out.fillBranch(
				"photon_eta", photons[tight_without_iso_sigmaietaieta_photons[0]].eta
			)
			self.out.fillBranch(
				"photon_phi", photons[tight_without_iso_sigmaietaieta_photons[0]].phi
			)
			self.out.fillBranch(
				"photon_vidNestedWPBitmap",
				photons[tight_without_iso_sigmaietaieta_photons[0]].vidNestedWPBitmap,
			)
			self.out.fillBranch(
				"photon_sieie",
				photons[tight_without_iso_sigmaietaieta_photons[0]].sieie,
			)
			self.out.fillBranch(
				"photon_pfRelIso03_all",
				photons[tight_without_iso_sigmaietaieta_photons[0]].pfRelIso03_all,
			)
			self.out.fillBranch(
				"photon_pfRelIso03_chg",
				photons[tight_without_iso_sigmaietaieta_photons[0]].pfRelIso03_chg,
			)
			if hasattr(
				photons[tight_without_iso_sigmaietaieta_photons[0]], "genPartFlav"
			):
				self.out.fillBranch(
					"photon_genPartFlav",
					photons[tight_without_iso_sigmaietaieta_photons[0]].genPartFlav,
			...
			if len(tight_muons) == 2 and len(tight_electrons) == 1:  # emumu channel
				if muons[tight_muons[0]].pdgId == -muons[tight_muons[1]].pdgId:
					if (muons[tight_muons[0]].pt >= 25) and (
						electrons[tight_electrons[0]].pt >= 25
					):
						dileptonmass = (
							muons[tight_muons[0]].p4() + muons[tight_muons[1]].p4()
						).M()
			...
			if len(tight_muons) == 1 and len(tight_electrons) == 2:
				if (
					electrons[tight_electrons[0]].pdgId
					== -electrons[tight_electrons[1]].pdgId
				):
					if (muons[tight_muons[0]].pt >= 25) and (
						electrons[tight_electrons[0]].pt >= 25
					):
			...
			if len(tight_electrons) == 3 and len(tight_muons) == 0:
				...
				if (
					electrons[tight_electrons[0]].charge
					== electrons[tight_electrons[1]].charge
				) and (
					electrons[tight_electrons[0]].charge
					== electrons[tight_electrons[2]].charge
				):
					...
					if (
						...
						tight_electrons[1], tight_electrons[2] = (
							tight_electrons[2],
							tight_electrons[1],
						)  # e.g. +-+ -> ++-
					elif (
						...
						tight_electrons[0], tight_electrons[2] = (
							tight_electrons[2],
							tight_electrons[0],
						)  # e.g. -++ -> ++-
					...
					mll13 = (
						electrons[tight_electrons[0]].p4()
						+ electrons[tight_electrons[2]].p4()
					).M()
					mll23 = (
						electrons[tight_electrons[1]].p4()
						+ electrons[tight_electrons[2]].p4()
					...
					if abs(mll13 - 91.188) > abs(mll23 - 91.188):
						...
						tight_electrons[0], tight_electrons[1] = (
							tight_electrons[1],
							tight_electrons[0],
						)  # move the w_lepton to the first one
						dileptonmass = mll13

					if (
						electrons[tight_electrons[1]].pt
						< electrons[tight_electrons[2]].pt
					):
						tight_electrons[1], tight_electrons[2] = (
							tight_electrons[2],
							tight_electrons[1],
					...
					if (electrons[tight_electrons[0]].pt >= 25) and (
						electrons[tight_electrons[1]].pt >= 25
					):
						channel = 3
			...
			if len(tight_muons) == 3 and len(tight_electrons) == 0:
				# move the different charge lepton to the end for further analysis
				if (muons[tight_muons[0]].charge == muons[tight_muons[1]].charge) and (
					muons[tight_muons[0]].charge == muons[tight_muons[2]].charge
				):
					pass
				else:
					if muons[tight_muons[0]].charge == muons[tight_muons[2]].charge:
						tight_muons[1], tight_muons[2] = (
							tight_muons[2],
							tight_muons[1],
						)  # e.g. +-+ -> ++-
					elif muons[tight_muons[1]].charge == muons[tight_muons[2]].charge:
						tight_muons[0], tight_muons[2] = (
							tight_muons[2],
							tight_muons[0],
						)  # e.g. -++ -> ++-
					...
					mll23 = -1.0
					mll13 = (
						muons[tight_muons[0]].p4() + muons[tight_muons[2]].p4()
					).M()
					mll23 = (
						muons[tight_muons[1]].p4() + muons[tight_muons[2]].p4()
					).M()
					if abs(mll13 - 91.188) > abs(mll23 - 91.188):
						dileptonmass = mll23
					else:
						tight_muons[0], tight_muons[1] = (
							tight_muons[1],
							tight_muons[0],
						)  # move the w_lepton to the first one
						dileptonmass = mll13
			...
			if channel == 1:
				...
				if hasattr(muons[tight_muons[0]], "genPartFlav"):
					self.out.fillBranch(
						"w_lepton_genPartFlav",
						electrons[tight_electrons[0]].genPartFlav,
					)
					self.out.fillBranch(
						"z_lepton1_genPartFlav", muons[tight_muons[0]].genPartFlav
					)
					self.out.fillBranch(
						"z_lepton2_genPartFlav", muons[tight_muons[1]].genPartFlav
					)
			elif channel == 2:
				...
				if hasattr(muons[tight_muons[0]], "genPartFlav"):
					self.out.fillBranch(
						"w_lepton_genPartFlav", muons[tight_muons[0]].genPartFlav
					)
					self.out.fillBranch(
						"z_lepton1_genPartFlav",
						electrons[tight_electrons[0]].genPartFlav,
					)
					self.out.fillBranch(
						"z_lepton2_genPartFlav",
						electrons[tight_electrons[1]].genPartFlav,
					)
			elif channel == 3:
				self.out.fillBranch("w_lepton_pt", electrons[tight_electrons[0]].pt)
				self.out.fillBranch("w_lepton_eta", electrons[tight_electrons[0]].eta)
				...
				if hasattr(electrons[tight_electrons[0]], "genPartFlav"):
					...
					self.out.fillBranch(
						"z_lepton2_genPartFlav",
						electrons[tight_electrons[2]].genPartFlav,
					)
			elif channel == 4:
				self.out.fillBranch("w_lepton_pt", muons[tight_muons[0]].pt)
				self.out.fillBranch("w_lepton_eta", muons[tight_muons[0]].eta)
				...
				if hasattr(muons[tight_muons[0]], "genPartFlav"):
					...
					self.out.fillBranch(
						"z_lepton1_genPartFlav", muons[tight_muons[1]].genPartFlav
					)
					self.out.fillBranch(
						"z_lepton2_genPartFlav", muons[tight_muons[2]].genPartFlav
					)
			self.out.fillBranch(
				"photon_pt", photons[tight_without_iso_sigmaietaieta_photons[0]].pt
			)
...
CR_FakePhotonFullModule_16preVFP = lambda: CR_FakePhotonFullProducer('2016_preVFP')
CR_FakePhotonFullModule_16 = lambda: CR_FakePhotonFullProducer('2016')
