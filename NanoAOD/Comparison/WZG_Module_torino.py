...
class WZG_Producer(Module):
	...
	def analyze(self, event):
		...
		for i in range(0,len(muons)):
			if event.Muon_corrected_pt[i] < 5: # Torino 
				continue
			if abs(muons[i].eta) > 2.4:
				continue

			if abs(muons[i].dxy) >= 0.5:
				continue

			if abs(muons[i].dz) >= 0.1:
				continue

			# thia make low eff
			#if event.Muon_tightCharge[i] == 2:
			#	continue

			if not ((event.Muon_isGlobal[i]) or (event.Muon_isTracker[i])):
				continue
			
			if event.Muon_corrected_pt[i] < 200:
				if muons[i].tightId and muons[i].pfRelIso03_all < 0.35 and (event.Muon_sip3d[i] < 4): 
					tight_muons.append(i)
			else:
				if (event.Muon_highPtId[i] == 1) and (muons[i].pfRelIso03_all < 0.35) and (event.Muon_sip3d[i] < 4): 
					tight_muons.append(i)
...
#('check: ', 0.3084616959095001)
#('check: ', -0.6765630841255188)
#('check: ', -0.4964769184589386)
#('check: ', -0.9960513114929199)
#('check: ', -0.7712141871452332)
#('check: ', -0.9884738326072693)
#('check: ', -0.9049723148345947)
#('check: ', 0.41990721225738525)
#('check: ', 0.5781444907188416)
#('check: ', -0.9735743999481201)
#('check: ', -0.9999241828918457)
#('check: ', -0.6045345664024353)
#('check: ', -0.9750617146492004)
#('check: ', -0.988241970539093)
#('check: ', -0.9954536557197571)




			print("check: ",event.Electron_mvaFall17V2noIso[i])
		...
		if len(tight_electrons)>=2:
			...
			if electrons[tight_electrons[0]].pt > 20:
...
WZG_select_Module_torino			= lambda : WZG_Producer()
WZG_select_Module_16preVFP_torino  = lambda : WZG_Producer("2016_preVFP")
WZG_select_Module_16_torino		= lambda : WZG_Producer("2016")
WZG_select_Module_17_torino		= lambda : WZG_Producer("2017")
WZG_select_Module_18_torino		= lambda : WZG_Producer("2018")
