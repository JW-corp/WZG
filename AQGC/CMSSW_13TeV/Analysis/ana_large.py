...
def Read_Tree(path):
	...
	basic_mask = (
		ak.num(Particles["Particle.PT"][ele_mask])
		+ ak.num(Particles["Particle.PT"][mu_mask])
	) >= 3
...
def Make_Object_Array(Particles):
	...
	Lepton_mask = (abs(Particles["Particle.PID"]) == 11) | (
		abs(Particles["Particle.PID"]) == 13
	)
	Lepton = ak.zip(
		{
...
def Analysis(Particles):
	...
	WLep_vec = vector.obj(
		px=Lepton[:, 0].Px, py=Lepton[:, 0].Py, pz=Lepton[:, 0].Pz, E=Lepton[:, 0].E
	)
	ZLep1_vec = vector.obj(
		px=Lepton[:, 1].Px, py=Lepton[:, 1].Py, pz=Lepton[:, 1].Pz, E=Lepton[:, 1].E
	)
	ZLep2_vec = vector.obj(
		px=Lepton[:, 2].Px, py=Lepton[:, 2].Py, pz=Lepton[:, 2].Pz, E=Lepton[:, 2].E
	)
	Pho_vec = vector.obj(
		px=Photon[:, 0].Px, py=Photon[:, 0].Py, pz=Photon[:, 0].Pz, E=Photon[:, 0].E
	)
	...
	return ak.to_numpy(MllA), ak.to_numpy(MlllA)
...
def Process(path):
	...
	for fin in flist:
		...
		if len(MllA) == 0:
			...
			mlla_tmp, mllla_tmp = Analysis(Particles)
	...
	return Total_gen_evts, MllA, MlllA
...
def draw_hist(df, target, start, end, bin, hist):
	...
	arr_w = (
		np.ones(len(llA_mass_arr)) * 35.86 * 1000 * df.loc[target]["xsec"] / Nevt
	)  # weight
	...
	bins_lla = [200, 400, 600, 1000, 1500, 2000, 5000]
...
if __name__ == "__main__":
	...
	# variable,mini,maxi = 'llA_mass',200,2000; Nbins=6 # histname, xmin xmax
	variable, mini, maxi = "lllA_mass", 300, 3000
	Nbins = 6  # histname, xmin xmax
