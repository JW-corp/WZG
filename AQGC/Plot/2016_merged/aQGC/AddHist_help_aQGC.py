...
def Make4Vec(target_arrays):
	...
	#Ele1vec =  vector.zip({'pt':lep1_pt,'eta':lep1_eta,'phi':lep1_phi,'M':lep1_m})
	#Ele2vec =  vector.zip({'pt':lep2_pt,'eta':lep2_eta,'phi':lep2_phi,'M':lep2_m})
	#Ele3vec =  vector.zip({'pt':lep3_pt,'eta':lep3_eta,'phi':lep3_phi,'M':lep3_m})
	#Phovec  =  vector.zip({'pt':pho_pt,'eta':pho_eta,'phi':pho_phi,'M':pho_m})
...
def AddHist(file, hist, isData, xsec, lumi, channel, branch):
	...
	if isData:
		...
		if file_basename == "LLWA_aQGC_UL16_Skim.root":
