...
def Plot():
	...
	for file in filelist_MC:
		...
		if file_basename == "aQGC":
			...
			for branch_name in branch:
				...
				hist_aQGC['m2']   = file_hist.Get(f'{channel_map[channel]}_{plot_branch}_{filelist_MC[file]["name"]}_{str(UpDown_map[UpDown])}_Rwt1')
				hist_aQGC['m15']   = file_hist.Get(f'{channel_map[channel]}_{plot_branch}_{filelist_MC[file]["name"]}_{str(UpDown_map[UpDown])}_Rwt2')
				hist_aQGC['m14']   = file_hist.Get(f'{channel_map[channel]}_{plot_branch}_{filelist_MC[file]["name"]}_{str(UpDown_map[UpDown])}_Rwt3')
				hist_aQGC['m13']   = file_hist.Get(f'{channel_map[channel]}_{plot_branch}_{filelist_MC[file]["name"]}_{str(UpDown_map[UpDown])}_Rwt4')
				hist_aQGC['m12']   = file_hist.Get(f'{channel_map[channel]}_{plot_branch}_{filelist_MC[file]["name"]}_{str(UpDown_map[UpDown])}_Rwt5')
	...
	for branch_name in branch:
		...
		hist_aQGC['m15'].SetLineColor(3)
		hist_aQGC['m15'].SetFillColor(0)
		hist_aQGC['m15'].SetLineWidth(4)

		hist_aQGC['m14'].SetLineColor(4)
		hist_aQGC['m14'].SetFillColor(0)
		hist_aQGC['m14'].SetLineWidth(4)

		hist_aQGC['m13'].SetLineColor(5)
		hist_aQGC['m13'].SetFillColor(0)
		hist_aQGC['m13'].SetLineWidth(4)

		hist_aQGC['m12'].SetLineColor(6)
		hist_aQGC['m12'].SetFillColor(0)
		hist_aQGC['m12'].SetLineWidth(4)
		...
		aQGC_m15		=	hist_aQGC['m15'].GetBinContent(bin)
		aQGC_m14		=	hist_aQGC['m14'].GetBinContent(bin)
		aQGC_m13		=	hist_aQGC['m13'].GetBinContent(bin)
		aQGC_m12		=	hist_aQGC['m12'].GetBinContent(bin)
		...
		#legend.AddEntry(hist_aQGC['m15']  ,f'aQGC cW -1.5 : {format(aQGC_m15,".3f")}','L')
		#legend.AddEntry(hist_aQGC['m14']  ,f'aQGC FM0 -1.4: {format(aQGC_m14,".3f")}','L')
		legend.AddEntry(hist_aQGC['m13']  ,f'aQGC FM0 -1.3: {format(aQGC_m13,".3f")}','L')
		legend.AddEntry(hist_aQGC['m12']  ,f'aQGC FM0 -1.2 {format(aQGC_m12,".3f")}','L')
		...
		if not (channel in [0,1,2,3,4]):
			...
			stack_mc.SetMaximum(7*hist_aQGC['m14'].GetMaximum())
			stack_mc.Draw("HIST")
			hist_aQGC['All_zero'].Draw("HIST SAME")
			#hist_aQGC['m15'].Draw("HIST SAME")
			#hist_aQGC['m14'].Draw("HIST SAME")
			hist_aQGC['m13'].Draw("HIST SAME")
			hist_aQGC['m12'].Draw("HIST SAME")
			# <<
