...
def Make4Vec(target_arrays):
	...
	Ele1vec =  vector.obj(pt=lep1_pt,eta=lep1_eta,phi=lep1_phi,M=lep1_m)
	Ele2vec =  vector.obj(pt=lep2_pt,eta=lep2_eta,phi=lep2_phi,M=lep2_m)
	Ele3vec =  vector.obj(pt=lep3_pt,eta=lep3_eta,phi=lep3_phi,M=lep3_m)
	Phovec  =  vector.obj(pt=pho_pt,eta=pho_eta,phi=pho_phi,M=pho_m)
	lllA_vec = Ele1vec + Ele2vec + Ele3vec + Phovec
	wa_vec = Ele1vec + Phovec

	# Please modify here to output other 4vectors
	return lllA_vec, wa_vec




def HLT_cut(file, branches,isData):
	# Single Electron HLT
	...
	if (~isData) and ('MuonEG_Run2016H' in file):
...
def AddHist(file, hist, isData, xsec, lumi, channel, branch):
	...
	if isData:
		...
		if file_basename == "LLWA_aQGC_2016.root":
	...
	arrays = HLT_cut(file, branches,isData)
	# >> Add lllA mass calculation
	lllA_vec,wa_vec = Make4Vec(arrays)
	arrays["WZG_mllla"] = lllA_vec.mass
	arrays["mwa"] = wa_vec.mass
	# <<--- Added my Jiwoong
	...
	if isData:
		if channel in [0,1,2,3,4, 10,11,12,13,14, 20,21,22,23,24, 30,31,32]:
			...
			MET_cut = (arrays.loc[:,'MET'] <= 30) #original
			#MET_cut = (arrays.loc[:,'MET'] > 0) #for test
		else:
		...
		if file_basename == "LLWA_aQGC_2016.root":
			...
			for i in range(len_weight):
				...
				# Varying Reweighting-Wegiht using this code block	
				...
				#arrays[f'true_weight_Rwt{i}'] = lumi * xsec * 1000 / true_events
				if channel in [0,1,2,3,4, 10,11,12,13,14, 20,21,22,23,24, 30,31,32]:
					MET_cut = (arrays.loc[:,f'MET_T1Smear_pt'] > 30)
					arrays_copy_nominal = arrays.loc[MET_cut,:].copy()
				elif channel in [5,6,7,8,9]:
					MET_cut = (arrays.loc[:,f'MET_T1Smear_pt'] <= 30) #original
					#MET_cut = (arrays.loc[:,f'MET_T1Smear_pt'] > 0) #for test
					arrays_copy_nominal = arrays.loc[MET_cut,:].copy()
				else:
					arrays_copy_nominal = arrays.copy()
			

				#print(arrays_copy_nominal[f'true_weight_Rwt{i}'])
				#print(arrays_copy_nominal['Muon_ID_Weight_DOWN'])
				print(arrays_copy_nominal[f'true_weight_Rwt{i}'][:,0].sum())
				...
				for UpDown in range(1,5):
					...
					if channel in [0,1,2,3,4, 10,11,12,13,14, 20,21,22,23,24, 30,31,32]:
						...
						MET_cut = (arrays.loc[:,f'MET_T1Smear_pt_{UpDown_map[UpDown]}'] <= 30) # original
						#MET_cut = (arrays.loc[:,f'MET_T1Smear_pt_{UpDown_map[UpDown]}'] > 0) # for test
						arrays_copy = arrays.loc[MET_cut,:].copy()
					else:
		...
		#------------->  Non-Reweight
			...
			if channel in [0,1,2,3,4, 10,11,12,13,14, 20,21,22,23,24, 30,31,32]:
				...
				MET_cut = (arrays.loc[:,f'MET_T1Smear_pt'] <= 30) #oroginal
				#MET_cut = (arrays.loc[:,f'MET_T1Smear_pt'] > 0) #for test
				arrays_copy_nominal = arrays.loc[MET_cut,:].copy()
			else:
				arrays_copy_nominal = arrays.copy()
			...
			for branch_name in branch:
				...
				print("HERE"*20)
				print(arrays_copy_nominal[branch[branch_name]["name"]])
...
def SetHistStyle(hist, color, isline=False):
	if color != 1:
