...
class WZG_plot():
	...
	def AddHist_FakeLep(self, file, hists={}, isData=True, xsec=0, **kwargs):
		time_init = time.time()
		init_branches = ['fake_lepton_weight','fake_lepton_weight_stat_up','fake_lepton_weight_stat_down','fake_lepton_weight_ele_up','fake_lepton_weight_ele_down','fake_lepton_weight_mu_up','fake_lepton_weight_mu_down']
		init_branches = self.init_branch(init_branches)
		...
		if isData:
			...
			for branch_name in self.branch:
				...
				if f'{branch_name}' in hists.keys():
					hists[branch_name] += h_temp
					hists[f'{branch_name}_fakerateStatUp'] += h_temp_up
					hists[f'{branch_name}_fakerateStatDown'] += h_temp_down
					# SysUnandDown added
					hists[f'{branch_name}_fakerateEleUp'] += h_temp_ele_up
					hists[f'{branch_name}_fakerateEleDown'] += h_temp_ele_down
					hists[f'{branch_name}_fakerateMuUp'] += h_temp_mu_up
					hists[f'{branch_name}_fakerateMuDown'] += h_temp_mu_down
				else:
					hists[branch_name] = deepcopy(h_temp)
					hists[f'{branch_name}_fakerateStatUp'] = deepcopy(h_temp_up)
					hists[f'{branch_name}_fakerateStatDown'] = deepcopy(h_temp_down)
					# SysUnandDown added
					hists[f'{branch_name}_fakerateEleUp'] = deepcopy(h_temp_ele_up)
					hists[f'{branch_name}_fakerateEleDown'] = deepcopy(h_temp_ele_down)
					hists[f'{branch_name}_fakerateMuUp'] = deepcopy(h_temp_mu_up)
					hists[f'{branch_name}_fakerateMuDown'] = deepcopy(h_temp_mu_down)
				del h_temp
				del h_temp_up
				del h_temp_down
				del h_temp_ele_up
	...
	def hist_store(self, hists_data={}, hists_mc={}, hists_flep={}, hists_fpho={}, **kwargs):
		...
		for branch_name in self.branch:
			...
			output[f'{self.channel_map[self.channel]}_{plotbranch}_FakeLep_fakerate_{year_suffix}StatUp'] = hists_flep[f'{branch_name}_fakerateStatUp']
			output[f'{self.channel_map[self.channel]}_{plotbranch}_FakeLep_fakerate_{year_suffix}StatDown'] = hists_flep[f'{branch_name}_fakerateStatDown']
			output[f'{self.channel_map[self.channel]}_{plotbranch}_FakeLep_fakerate_{year_suffix}EleUp'] = hists_flep[f'{branch_name}_fakerateEleUp']
			output[f'{self.channel_map[self.channel]}_{plotbranch}_FakeLep_fakerate_{year_suffix}EleDown'] = hists_flep[f'{branch_name}_fakerateEleDown']
			output[f'{self.channel_map[self.channel]}_{plotbranch}_FakeLep_fakerate_{year_suffix}MuUp'] = hists_flep[f'{branch_name}_fakerateMuUp']
			output[f'{self.channel_map[self.channel]}_{plotbranch}_FakeLep_fakerate_{year_suffix}MuDown'] = hists_flep[f'{branch_name}_fakerateMuDown']
