...
def auto_cal(region, file, variable, process, uncertainty, bin,idx=None):
	...
	if 'fakerate_18sta' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_fakerate_18StatUp')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_fakerate_18StatDown')).GetBinContent(bin)
		return process_unc('FakeLepton Stat',nominal, up, down)
	
	if 'fakerate_18ele' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_fakerate_18EleUp')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_fakerate_18EleDown')).GetBinContent(bin)
		return process_unc('FakeLepton Ele',nominal, up, down)
	
	if 'fakerate_18mu' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_fakerate_18MuUp')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_fakerate_18MuDown')).GetBinContent(bin)
		return process_unc('FakeLepton Mu',nominal, up, down)


	if 'fakepho_mc' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_FakePho_mcUp')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_FakePho_mcDown')).GetBinContent(bin)
		return process_unc('FakePhoton MC',nominal, up, down)

	if 'fakepho_closure' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_FakePho_closureUp')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_FakePho_closureDown')).GetBinContent(bin)
		return process_unc('FakePhoton Closure',nominal, up, down)

	if 'fakepho_iso' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_FakePho_isoUp')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_FakePho_isoDown')).GetBinContent(bin)
		return process_unc('FakePhoton Iso',nominal, up, down)

	if 'fakepho_sta_18' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_FakePho_stat_18Up')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_FakePho_stat_18Down')).GetBinContent(bin)
		return process_unc('FakePhoton fraction Stat',nominal, up, down)
	...
	elif 'btagweight_bc_uncorr_18' in uncertainty.lower():
		nominal = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up = file.Get(str(hist_name + '_btagWeight_bc_uncorr_18Up')).GetBinContent(bin)
		down = file.Get(str(hist_name + '_btagWeight_bc_uncorr_18Down')).GetBinContent(bin)
		return process_unc('btagWeight_bc_uncorr', nominal, up, down)

	elif 'btagweight_l_uncorr_18' in uncertainty.lower():
		nominal = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up = file.Get(str(hist_name + '_btagWeight_l_uncorr_18Up')).GetBinContent(bin)
		down = file.Get(str(hist_name + '_btagWeight_l_uncorr_18Down')).GetBinContent(bin)
...
if __name__ == '__main__':
	...
	for region in jsons['regions']:
		...
		tag = jsons['regions'][region]['tag']  # year
		variable = jsons['regions'][region]['variable'] # physics variable
