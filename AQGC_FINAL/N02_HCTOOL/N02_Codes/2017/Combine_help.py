...
def auto_cal(region, file, variable, process, uncertainty, bin,idx=None):
	...
	if 'fakerate_17' in uncertainty.lower():
	...
	if 'fakerate_17sys' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_fakerate_17SysUp')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_fakerate_17SysDown')).GetBinContent(bin)
		return process_unc('FakeLepton Sys',nominal, up, down)
	...
	if 'fakepho_sta_17' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_FakePho_stat_17Up')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_FakePho_stat_17Down')).GetBinContent(bin)
	...
	if 'jestotal_17' in uncertainty.lower():
		...
		print("EleID: ",ElectronID_nominal,ElectronID_up,ElectronID_down)
