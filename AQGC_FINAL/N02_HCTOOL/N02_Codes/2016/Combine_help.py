...
def auto_cal(region, file, variable, process, uncertainty, bin,idx=None):
	...
	if 'fakerate_16' in uncertainty.lower():
	...
	if 'fakerate_16sys' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_fakerate_16SysUp')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_fakerate_16SysDown')).GetBinContent(bin)
	...
	if 'fakepho_sta_16' in uncertainty.lower():

		nominal   = file.Get(str(hist_name + '_None')).GetBinContent(bin)
		up   	  = file.Get(str(hist_name + '_FakePho_stat_16Up')).GetBinContent(bin)
		down 	  = file.Get(str(hist_name + '_FakePho_stat_16Down')).GetBinContent(bin)
